...
class Interpreter {
  ...
  Interpreter._(this._interpreter) {
    ...
    // Patched: models with a DYNAMIC input shape (e.g. openWakeWord's
    // melspectrogram) can't allocate until the input is resized — so defer
    // instead of throwing "failed precondition" on load. run()/allocateTensors
    // handle allocation after the caller resizes the input.
    try {
      allocateTensors();
    } catch (_) {
      _allocated = false;
